...
import HyperdriveAnimatedPage from "./HyperdriveAnimatedPage";
import type { HyperdriveConfig, HyperdriveService, HyperdriveFeature, HyperdriveInstructor, HyperdrivePrimaryColor } from "./HyperdriveConfig";
import { ICON_MAP, ICON_CATEGORIES } from "../cafe/CafeIcons";
import {
  Tag, Zap, ShieldCheck, Users, MapPin, Megaphone,
  Save, ChevronDown, ChevronUp, Upload, X, Search, LucideIcon,
} from "lucide-react";

// ── Icon picker (reused from cafe) ───────────────────────────────────────────

function HdIcon({ name, size = 16, className = "" }: { name: string; size?: number; className?: string }) {
  const Icon = ICON_MAP[name] as LucideIcon | undefined;
  if (!Icon) { const FB = ICON_MAP["Zap"] as LucideIcon; return <FB size={size} className={className} />; }
  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen((p) => !p)}
        ...
        <HdIcon name={value} size={16} className="text-slate-600" />
...
const SECTION_ICONS: Record<string, LucideIcon> = {
  brand: Tag, services: Zap, features: ShieldCheck,
  instructors: Users, contact: MapPin, cta: Megaphone,
};
...
const COLOR_OPTIONS: { value: HyperdrivePrimaryColor; label: string; dot: string }[] = [
...
export default function HyperdriveEditorPanel({
  config, onChange, saving, onSave,
}: {
  config: HyperdriveConfig; onChange: (c: HyperdriveConfig) => void; saving: boolean; onSave: () => void;
}) {
  const [openSection, setOpenSection] = useState<string>("brand");
  const toggle = (k: string) => setOpenSection((p) => (p === k ? "" : k));
  const set = <K extends keyof HyperdriveConfig>(key: K, val: HyperdriveConfig[K]) =>
    onChange({ ...config, [key]: val });

  // Services helpers
  const setService = (i: number, field: keyof HyperdriveService, val: string | boolean) =>
    set("services", config.services.map((s, idx) => idx === i ? { ...s, [field]: val } : s));
  const addService = () =>
    set("services", [...config.services, { icon: "Zap", name: "Шинэ сургалт", price: "0₮", duration: "1 цаг", desc: "", highlight: false }]);
  const removeService = (i: number) =>
    set("services", config.services.filter((_, idx) => idx !== i));

  // Features helpers
  const setFeature = (i: number, field: keyof HyperdriveFeature, val: string) =>
    set("features", config.features.map((f, idx) => idx === i ? { ...f, [field]: val } : f));
  const addFeature = () =>
    set("features", [...config.features, { icon: "Star", title: "Шинэ онцлог", desc: "" }]);
  const removeFeature = (i: number) =>
    set("features", config.features.filter((_, idx) => idx !== i));

  // Instructors helpers
  const setInstructor = (i: number, field: keyof HyperdriveInstructor, val: string) =>
    set("instructors", config.instructors.map((inst, idx) => idx === i ? { ...inst, [field]: val } : inst));
  const addInstructor = () =>
    set("instructors", [...config.instructors, { name: "Шинэ багш", role: "Instructor", exp: "1 жил" }]);
  const removeInstructor = (i: number) =>
    set("instructors", config.instructors.filter((_, idx) => idx !== i));

  // Hours helpers
  const setHours = (i: number, field: "day" | "time", val: string) =>
    set("hours", config.hours.map((h, idx) => idx === i ? { ...h, [field]: val } : h));
  const addHours = () =>
    set("hours", [...config.hours, { day: "Гараг", time: "09:00 – 18:00" }]);
  const removeHours = (i: number) =>
    set("hours", config.hours.filter((_, idx) => idx !== i));
    ...
    <PanelGroup direction="horizontal">
        ...
        <div className="h-full overflow-y-auto border-r border-slate-200 bg-slate-50">
          <div className="p-5 space-y-3">
            ...
            <div className="flex items-center justify-between mb-2">
              <div>
                <div className="text-sm font-bold text-slate-900">Hyperdrive засах</div>
            ...
            {openSection === "brand" && (
              <div className="space-y-3 px-1">
                ...
                <Field label="Брэндийн нэр">
                  <Input value={config.brandName} onChange={(e) => set("brandName", e.target.value)} placeholder="Hyperdrive Center" />
                </Field>
                ...
                <Field label="URL preview">
                  <div className="flex items-center rounded-xl border border-slate-200 bg-white overflow-hidden">
                    ...
                    <input value={config.slugPreview} onChange={(e) => set("slugPreview", e.target.value)}
                      className="flex-1 py-2 pr-3 text-xs outline-none text-slate-900" placeholder="hyperdrive-center" />
                  </div>
                </Field>
            ...
            {/* Services */}
            <SectionHeader id="services" title="Сургалтын багцууд" open={openSection === "services"} onToggle={() => toggle("services")} />
            {openSection === "services" && (
              <div className="space-y-3 px-1">
                <div className="grid grid-cols-2 gap-2">
                  <Field label="Гарчиг"><Input value={config.servicesTitle} onChange={(e) => set("servicesTitle", e.target.value)} /></Field>
                  <Field label="Дэд гарчиг"><Input value={config.servicesSubtitle} onChange={(e) => set("servicesSubtitle", e.target.value)} /></Field>
                </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <label className="flex items-center gap-1 text-xs text-slate-500 cursor-pointer">
                          <input type="checkbox" checked={!!svc.highlight}
                            onChange={(e) => setService(i, "highlight", e.target.checked)}
                            className="rounded" />
                          Алдартай
                        </label>
                        <button onClick={() => removeService(i)} className="text-xs text-rose-400 hover:text-rose-600">✕</button>
                      </div>
                    </div>
                    <div className="grid grid-cols-4 gap-1.5 items-end">
                      <div>
                        <div className="text-xs text-slate-500 mb-1">Icon</div>
                        <IconPicker value={svc.icon} onChange={(v) => setService(i, "icon", v)} />
                      </div>
                      <Input value={svc.name} onChange={(e) => setService(i, "name", e.target.value)} placeholder="Нэр" className="col-span-3" />
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
                      <Input value={svc.price} onChange={(e) => setService(i, "price", e.target.value)} placeholder="150,000₮" />
                      <Input value={svc.duration} onChange={(e) => setService(i, "duration", e.target.value)} placeholder="2 цаг" />
                    </div>
                    <Input value={svc.desc} onChange={(e) => setService(i, "desc", e.target.value)} placeholder="Тайлбар" />
                  </div>
                ))}
                <Button size="sm" variant="secondary" onClick={addService} className="w-full">+ Сургалт нэмэх</Button>
              </div>
            )}
              ...
              <div className="space-y-3 px-1">
                {config.instructors.map((inst, i) => (
                  <div key={i} className="rounded-xl border border-slate-200 bg-white p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      <Input value={inst.role} onChange={(e) => setInstructor(i, "role", e.target.value)} placeholder="Албан тушаал" />
                      <Input value={inst.exp} onChange={(e) => setInstructor(i, "exp", e.target.value)} placeholder="10 жил" />
                    </div>
                  </div>
                ))}
            ...
            {openSection === "contact" && (
              <div className="space-y-3 px-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Field label="Утас"><Input value={config.phone} onChange={(e) => set("phone", e.target.value)} placeholder="99001122" /></Field>
      ...
      <Panel defaultSize={62} minSize={40}>
        <div className="h-full overflow-y-auto bg-zinc-950">
          <div className="sticky top-0 z-10 border-b border-white/10 bg-zinc-900 px-4 py-2.5 flex items-center gap-3">
            <div className="flex gap-1.5">
              <div className="h-3 w-3 rounded-full bg-rose-400" />
              <div className="h-3 w-3 rounded-full bg-amber-400" />
              <div className="h-3 w-3 rounded-full bg-emerald-400" />
            </div>
            <div className="flex-1 rounded-md bg-zinc-800 border border-white/10 px-3 py-1 text-xs text-zinc-400 text-center">
              landing.mn/p/{config.slugPreview} — Live preview
            </div>
          </div>
          <HyperdriveAnimatedPage config={config} />
        </div>
      </Panel>
    </PanelGroup>
  );
